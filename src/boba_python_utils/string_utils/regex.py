...
from boba_python_utils.common_utils.iter_helper import product_
from boba_python_utils.common_utils.typing_helper import all_str
...
def get_contains_whole_word_regex(word: Union[str, Iterable[str]], escape: bool = True, additional_seps: str = '_') -> str:
    ...
    Examples:
        >>> print(get_contains_whole_word_regex('ca-fe'))
        (?:\\b|_)ca\\-fe(?:\\b|_)
        >>> print(get_contains_whole_word_regex('ca-fe', escape=False))
        (?:\\b|_)ca-fe(?:\\b|_)
        >>> print(get_contains_whole_word_regex(['cafe', 'bar'], additional_seps='_-'))
        (?:\\b|_|\\-)(cafe|bar)(?:\\b|_|\\-)
    """
    if additional_seps:
