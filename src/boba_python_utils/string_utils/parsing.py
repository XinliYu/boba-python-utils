...
from boba_python_utils.common_utils import zip__, zip_longest__
from boba_python_utils.string_utils.misc import get_domain_from_name
...
def split_function_calls(input_string: str, separator: str = ';'):
    """
    ...
    for char in input_string:
        if char in ['"', "'"]:
            if not in_quote:
                in_quote = True
                quote_char = char
            elif char == quote_char and current_call[-1] != '\\':
                in_quote = False
...
def get_function_names(invocation_string: str, separator: Optional[str] = None) -> Union[str, List[str]]:
    if separator:
        return [
        ...
        return invocation_string.split('(', maxsplit=1)[0].strip()
