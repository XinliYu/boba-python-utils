...
from os import path
from typing import Union, List, Dict, Any, Mapping, Optional, Callable, Tuple
import sys

from boba_python_utils.common_utils.array_helper import is_homogeneous_sequence
from boba_python_utils.general_utils.arg_utility.arg_naming import solve_parameter_info, solve_arg_full_and_short_name
from boba_python_utils.common_utils.environment_helper import path_import, is_ipython
from boba_python_utils.common_utils.slot_tuple import NamedTuple
from boba_python_utils.common_utils.typing_helper import bool_, nonstr_iterable, element_type, map_iterable_elements
from boba_python_utils.general_utils.console_util import hprint_message
from boba_python_utils.string_utils.parsing import PreDefinedArgConverters, parse_with_predefined_convert
...
def dict_to_namespace(d, recursive=True):
    ...
    if recursive:
        return Namespace(
            **{
                k: (dict_to_namespace(v, recursive=True)
        ...
        if action.dest == arg_name:
            ...
            if verbose:
                hprint_message(
                    ...
                    'needs_update_default_value', needs_update_default_value,
                    title=f'default value update for {arg_name}'
                )
            if needs_update_default_value:
                action.default = arg_default_value
                if (
                        set_required_to_false_if_default_value_is_not_none and
                        arg_default_value is not None
                ):
...
def get_seen_actions(arg_parser: ArgumentParser, argv: List[str] = None) -> List[str]:
    ...
    seen_actions = [
        action.dest
        for action in arg_parser._actions
        if any(option_string in specified_options for option_string in action.option_strings)
    ]
    return seen_actions
...
def get_unseen_actions(arg_parser, argv: List[str] = None):
    ...
    unseen_actions = [
        action.dest
        for action in arg_parser._actions
        if not any(option_string in specified_options for option_string in action.option_strings) and not isinstance(action, _HelpAction)
    ]
...
def get_parsed_args(
            ...
            if preset_path is not None:
                if is_python:
                    preset_obj = {
                        ...
                        for k, v in path_import(path.abspath(preset_path)).config.items()}
                else:
    ...
    if verbose:
        hprint_message(
            'arg_parser._actions', arg_parser._actions,
            'num_existing_args', len(arg_parser._actions),
    ...
    def _add_arg():
        nonlocal converter
        if exposed_args and arg_full_name not in exposed_args:  # we hide non-exposed args
            hidden_args.append((arg_full_name, default_value))
        else:
            _description = description if description else ''
            disable_default_value = False
            if not _is_ipython and (required_args is not None and arg_full_name in required_args):
                disable_default_value = True
            elif _description:
                _description += f"; the default value is " + (
                    f"'{default_value}'" if isinstance(default_value, str)
    ...
    for arg_info_obj in arg_info_objs:
        arg_full_name, arg_short_name, default_value, converter, description = solve_parameter_info(
        ...
        if sanitized_arg_name != arg_full_name:
            if sanitized_arg_name not in sanitized_arg_full_name_to_original_arg_full_name_map:
                sanitized_arg_full_name_to_original_arg_full_name_map[sanitized_arg_name] = \
                    arg_full_name
            else:
                raise ValueError(
        ...
        for arg_full_name, default_value in preset.items():
            ...
            if arg_full_name:
                if default_value is None:
                    default_value = kwargs.get(arg_full_name, kwargs.pop(default_value_prefix + arg_full_name, None))
                _add_arg()
    ...
    if exposed_args and hidden_args:
        for arg_full_name, arg_val in hidden_args:
            setattr(args, arg_full_name, _proc_arg_val(arg_val, constants, force_parse_boolstr))

    for arg_full_name, arg_val in vars(args).items():
        ori_arg_name = sanitized_arg_full_name_to_original_arg_full_name_map.get(arg_full_name, arg_full_name)
        if isinstance(arg_val, str):
...
def get_args(preset=None, argv=None, **kwargs) -> Namespace:
    ...
    defaults = {key[8:]: value for key, value in kwargs.items() if key.startswith('default_')}
    ...
    for arg, value in combined_defaults.items():
        if isinstance(value, list) and is_homogeneous_sequence(value):
            parser.add_argument(f'--{arg}', nargs='*', type=type(value[0]) if value else str, default=value)
        elif isinstance(value, tuple) and is_homogeneous_sequence(value):
            parser.add_argument(f'--{arg}', nargs='*', type=type(value[0]) if value else str, default=list(value),
                                action=lambda x: tuple(x))
        else:
            parser.add_argument(f'--{arg}', type=json.loads if isinstance(value, str) and (value.startswith('[') or value.startswith('{')) else type(value), default=value)
